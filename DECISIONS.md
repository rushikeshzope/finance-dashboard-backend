...
I eliminated raw `try/catch` enclosures from all controllers by wrapping them in a higher-order `asyncHandler`.
...
For importing transactions, I utilized the `csv-parser` package acting on a direct `Readable` stream piped from Multer's in-memory buffer.
...
## 6. Database Selection: PostgreSQL
* **Why?** PostgreSQL was chosen as the database provider for this project because it is a robust, production-ready relational database. Because Prisma is our ORM, our data access logic remains completely generic and decoupled from the underlying database engine!
